#if __has_include(<WebKit/DOMCSSStyleDeclaration.h>)
	...
	for (size_t n = 0; n <= len; n++) {
		...
		if (c == ':') {
            ...
            if( accumIdx > 0 ) //if there is a ';' and '\0' to end the style, avoid adding an empty key-value pair
            {
				...
				DOMCSSValue *cssValue;
				if( [cssValueString rangeOfString:@" "].length > 0 )
					cssValue = [[DOMCSSValueList alloc] init];
				else
					cssValue = [[DOMCSSPrimitiveValue alloc] init];
...
{
	DOMCSSValue* v = [self getPropertyCSSValue:propertyName];
...
-(NSString*) item:(long) index
{
	...
	DOMCSSValue* v = [sortedKeys objectAtIndex:index];
	return v.cssText;
}
