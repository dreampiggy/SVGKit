...
-(float) getFloatValue:(DOMCSSPrimitiveType) unitType
{
	...
	switch( self.primitiveType )
	{
		...
		case DOM_CSS_NUMBER:
		{
			if( unitType == DOM_CSS_PX ) /** Dom 1 spec allows this, SVG Spec says "this is correct by spec", and DOM 2 spec says this is illegal. Most CSS interpreters do it... */
			{
				return self.internalValue;
			}
			else
...
-(void)setCssText:(NSString *)newCssText
{
	...
	|| _cssText.length == 0 )
	{
		self.internalValue = 0.0f;
		self.internalString = @"";
		self.primitiveType = DOM_CSS_UNKNOWN;
	}
	else if( [_cssText hasSuffix:@"%"])
		[self setFloatValue:DOM_CSS_PERCENTAGE floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"em"])
		[self setFloatValue:DOM_CSS_EMS floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"ex"])
		[self setFloatValue:DOM_CSS_EXS floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"px"])
		[self setFloatValue:DOM_CSS_PX floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"cm"])
		[self setFloatValue:DOM_CSS_CM floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"mm"])
		[self setFloatValue:DOM_CSS_MM floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"in"])
		[self setFloatValue:DOM_CSS_IN floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"pt"])
		[self setFloatValue:DOM_CSS_PT floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"pc"])
		[self setFloatValue:DOM_CSS_PC floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"deg"])
		[self setFloatValue:DOM_CSS_DEG floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"rad"])
		[self setFloatValue:DOM_CSS_RAD floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"grad"])
		[self setFloatValue:DOM_CSS_GRAD floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"ms"])
		[self setFloatValue:DOM_CSS_MS floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"s"])
		[self setFloatValue:DOM_CSS_S floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"khz"]) // -----------NB: check this before checking HZ !
		[self setFloatValue:DOM_CSS_KHZ floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"hz"])
		[self setFloatValue:DOM_CSS_HZ floatValue:[_cssText floatValue]];
	else
	{
		/**
		...
		if( [scanner scanFloat:&floatToHoldTheOutput])
		{
			...
			[self setFloatValue:DOM_CSS_NUMBER floatValue:floatToHoldTheOutput];
		}
		else
		{
...
#endif
			[self setStringValue:DOM_CSS_STRING stringValue:_cssText]; // -------- NB: we allow any string-to-string conversion, so it's not a huge problem that we dont correctly detect "url" versus "other kind of string". I hate CSS Parsing...
		}
	}
}
