...
-(long)insertRule:(NSString *)rule index:(unsigned long)index
{
	...
	{
		DOMCSSStyleRule* newRule = [[DOMCSSStyleRule alloc] initWithSelectorText:[stringSplitContainer objectAtIndex:0] styleText:[stringSplitContainer objectAtIndex:1]];
...
- (id)initWithString:(NSString*) styleSheetBody
{
    ...
    if (self)
	{
		self.cssRules = [[DOMCSSRuleList alloc]init];
