	...
	{	
		...
		{	
			...
			if( [hrefAttribute length] > 0 )
			{
				...
				Element* linkedElement = [parseResult.parsedDocument getElementById:linkHref];
                
                NSAssert( linkedElement != nil, @"Found an SVG <use> tag that points to a non-existent element. Missing element: id = %@", linkHref );
                
                if ([linkedElement isKindOfClass:[SVGElement class]]) {
                    useElement.instanceRoot = [self convertSVGElementToElementInstanceTree:(SVGElement *)linkedElement outermostUseElement:useElement];
                }
			}
